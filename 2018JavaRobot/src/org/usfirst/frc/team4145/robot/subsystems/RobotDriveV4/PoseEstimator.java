...
public class PoseEstimator {
    ...
    protected InterpolatingTreeMap<InterpolatingDouble, RigidTransform2d> fieldToVehicle, test;
    ...
    public void reset(double start_time, RigidTransform2d initial_field_to_vehicle){
        ...
        test = new InterpolatingTreeMap<>(Constants.OBSERVATION_BUFFER_SIZE);
    ...
    public synchronized Map.Entry<InterpolatingDouble, RigidTransform2d> getLatestTest(){
    ...
    private Runnable loop = () -> {
        ...
        addObservations(currentTime, odometry, transOdometry);
    ...
    public synchronized void addObservations(double timestamp, RigidTransform2d observation, RigidTransform2d testTransform) {
        fieldToVehicle.put(new InterpolatingDouble(timestamp), observation);
        test.put(new InterpolatingDouble(timestamp), testTransform);
    }
    ...
    public void outputToSmartDashboard() {
        ...
        RigidTransform2d testOdom = getLatestTest().getValue();
        SmartDashboard.putNumber("Robot Pose X2", testOdom.getTranslation().getX());
        SmartDashboard.putNumber("Robot Pose Y2", testOdom.getTranslation().getY());
        SmartDashboard.putNumber("Robot Pose Theta2", testOdom.getRotation().getDegrees());
    }
